...
def main():
    ...
    if 'llama2' in args.model_name_or_path:
        ...
        tokenizer = AutoTokenizer.from_pretrained(args.model_name_or_path, use_fast=True, legacy=False)
    ...
    def batcher(params, batch, max_length=None):
        ...
        #pad_mask = batch["attention_mask"].eq(0)
        #if pad_mask.any():
            #pad_ids = batch["input_ids"][pad_mask]
            #uniq = torch.unique(pad_ids).tolist()
            #print("PAD ids in padded positions:", uniq)
            #print("pad_token_id:", tokenizer.pad_token_id, "eos_token_id:", tokenizer.eos_token_id)
            #if tokenizer.eos_token_id in uniq:
                #print("!!! WARNING: eos_token_id appears in padding positions -> padding uses EOS")
